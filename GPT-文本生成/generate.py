...
def sample_sequence(model, context, length, n_ctx, temperature=1.0, top_k=30, top_p=0.0, repitition_penalty=1.0,
                    device='cpu'):
...
def generate(n_ctx, model, context, length, temperature=1, top_k=0, top_p=0.0, repitition_penalty=1.0, device='cpu',
             is_fast_pattern=False):
    if is_fast_pattern:
        return fast_sample_sequence(model, context, length, temperature=temperature, top_k=top_k, top_p=top_p,
                                    device=device)
    else:
        return sample_sequence(model, context, length, n_ctx, temperature=temperature, top_k=top_k, top_p=top_p,
...
def main():
    ...
    parser.add_argument('--model_path', default='model/model_epoch29', type=str, required=False, help='模型路径')
    parser.add_argument('--prefix', default='月光', type=str, required=False, help='生成文章的开头')
